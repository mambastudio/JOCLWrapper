...
#define getInputF(index) (getDataF(index, length, data))
#define setOutputF(index, value) (setDataF(index, value, length, data))
...
void setData(int index, float value, global int* length, global int* data)
{
    if(index >= *length)
        return;
    else
...
int getDataF(int index, global int* length, global float* data)
{
    if(index >= *length)
        return 0;
    else
...
void setDataF(int index, float value, global int* length, global float* data)
{
    if(index >= *length)
        return;
    else
                                 ...
                                 local  float* temp)
{
     ...
     temp [local_id] = getInputF(global_id);
    ...
    barrier (CLK_LOCAL_MEM_FENCE);
    setOutputF(global_id, temp[local_id]);
}
